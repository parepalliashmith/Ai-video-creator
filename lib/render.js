...
const FONTS = {
  en: path.join(__dirname, '..', 'assets', 'fonts', 'Caption.ttf'),
  te: path.join(__dirname, '..', 'assets', 'fonts', 'Caption-Telugu.ttf'),
  hi: path.join(__dirname, '..', 'assets', 'fonts', 'Caption-Hindi.ttf'),
};
...
async function renderSceneClip({ imagePath, narrationPath, duration, captionText, lang }, index, orientation, workDir) {
  ...
  if (captionText && captionText.trim()) {
    ...
    fs.writeFileSync(captionFile, captionText.trim());
    const fontSize = Math.round(dims.h * 0.045);
    const bottomMargin = Math.round(dims.h * 0.09);
    const fontPath = FONTS[lang] || FONTS.en;
    filters.push(
      `drawtext=fontfile='${escFilterPath(fontPath)}':textfile='${escFilterPath(captionFile)}':` +
        `fontcolor=white:fontsize=${fontSize}:borderw=3:bordercolor=black@0.8:` +
        `x=(w-text_w)/2:y=h-th-${bottomMargin}:line_spacing=6`
    );
  }
