...
async function xfadeConcat(clipPaths, durations, workDir, outPath) {
  ...
  // Cycle through several of ffmpeg's built-in xfade transition styles rather
  // than a single repeated fade, so a multi-scene video feels more varied.
  // All of these are standard xfade transitions (safe/well-supported), no
  // extra filter dependency beyond xfade itself.
  const TRANSITION_STYLES = ['fade', 'dissolve', 'wipeleft', 'slideup', 'circleopen', 'wiperight', 'slideleft'];
  ...
  for (let i = 1; i < n; i++) {
    ...
    const style = TRANSITION_STYLES[(i - 1) % TRANSITION_STYLES.length];

    const filterLines = [
      `[0:v][1:v]xfade=transition=${style}:duration=${d.toFixed(3)}:offset=${offset.toFixed(3)}[vx]`,
...
// License-free music beds, synthesized directly as WAV in plain JS — this
// static ffmpeg build doesn't compile in the `lavfi` virtual input, so tone
// generation can't go through ffmpeg at all here. Three moods, each just a
// different chord + optional amplitude pulse (no samples/loops needed).
const MUSIC_MOODS = {
  calm: { freqs: [220, 277.18, 329.63], pulseHz: 0, amplitude: 0.22 },
  upbeat: { freqs: [261.63, 329.63, 392.0, 440.0], pulseHz: 2.4, amplitude: 0.22 },
  dramatic: { freqs: [110, 220, 261.63, 329.63], pulseHz: 0.5, amplitude: 0.27 },
};

function writeWavSineBed(outPath, duration, mood = 'calm') {
  const { freqs, pulseHz, amplitude } = MUSIC_MOODS[mood] || MUSIC_MOODS.calm;
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    if (pulseHz > 0) envelope *= 0.75 + 0.25 * Math.sin(2 * Math.PI * pulseHz * t);
...
function generateMusicBed(duration, outPath, mood) {
  writeWavSineBed(outPath, Math.max(1, duration), mood);
...
export async function renderVideo({ scenes, orientation, addMusic, musicMood, fastMode, workDir, outPath, onProgress }) {
  ...
  await generateMusicBed(totalDuration, musicPath, musicMood);
